...
x <- tibble(
  results = map_int(1:n, ~ roll_two_dice()),
  results2 = map_int(1:n, ~ roll_two_dice())
  ) %>% 
  mutate(craps1 = results %in% c(7,11)) %>% 
  mutate(craps2 = results2 %in% c(7,11)) %>% 
  mutate(winner = ifelse(
    craps1 & craps2, 'Tie', ifelse(
      craps1, 'IB', ifelse(
         craps2, 'JM', 'no winner')
      )
    )
